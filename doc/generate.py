...
############### Отдельная замена: прямо в файле processor.h ##################

fileProcessorH = "../EScript/processor.h"
phContent = ""
with open(fileProcessorH, 'r') as f:
    phContent = f.read()
...
if iEnd < 0:
    ...
    sys.exit()
sContent = phContent[0:iStart+len(startPattern)] +"\n" + strOpCodesRunCaseContent + phContent[iEnd:]

with codecs.open(fileProcessorH, "w", "utf-8") as temp:
    temp.write(sContent)
