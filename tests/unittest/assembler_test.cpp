...
void Assembler_Test::test_byteSequence()
{
    ...
    uint64_t_value = rand();
    a.ctor(uint64_t_value);
    p = obj.data() + n;
    n += Assembler::instructionSize(OpCode::CTOR);
    assert(*p == (OpCodeType)OpCode::CTOR);
    p += Assembler::INSTRUCTION_CODE_SIZE;
    assert(*((uint64_t *)p) == uint64_t_value);
    assert(obj.size() == n);
    
    uint8_t_value = rand();
    a.chtype(uint8_t_value);
    p = obj.data() + n;
    n += Assembler::instructionSize(OpCode::CHTYPE);
    assert(*p == (OpCodeType)OpCode::CHTYPE);
    p += Assembler::INSTRUCTION_CODE_SIZE;
    assert(*((uint8_t *)p) == uint8_t_value);
    assert(obj.size() == n);
