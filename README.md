...
Например, в данный момент нет такого понятия, как `undefined`, использован будет только `null`,
т.к. две сущности по сути выполняют одну работу, указывая на нечно несуществующее.
Как в C++ достаточно `nullptr`, так и в Eugen-Script будет только `null` и никакого `undefined`.

Все неинициализированные переменные устанавливаются в `null`.
```javascript
var a;
b = a == null; // -> b == true
```
Каждая переменная - это ссылка на что-то или равна `null`.
...
Деление `/` - целочисленное. Если нужно вещественный результат, нужно использовать 
