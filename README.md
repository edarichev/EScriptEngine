...
мы работали в стандарной 16-битовой реализации:
...
Например, так массив можно создать и так:
```javascript
...
a = [12, 34, 56, "hello"];
```
Объект `DateTime` тое можно создать оператором `new`.
...
|Метод|Описание|
|-|-|
|new DateTime(year, month, day, hour, minute, second, millisecond)|Создаёт и возвращает экземпляр DateTime с указанными параметрами. Параметры необязательны: месяц ставится в 1, день - в 1, часы, минуты, секунды - в 0. Если не указано ничего, возвращается текущая дата и время.|
...
если этот объект вам ещё понадобится после удаления движка. По умолчанию она равноа `false`,
...
Вероятно, вы захотите передать туда какие-то аргументы, это делается о уже 
...
5. Перед запуском скрипта зарегистрируйте имя класса и функцию создания объекта методом
`EScript::registerClass`, например, так это сделано для того же класса `Array`:
```C++
registerClass(U"Array", Array::ctor);
```
После этого транслятор найдёт этот класс и сгенерирует вызов нужной функции.
