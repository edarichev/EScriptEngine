...
`fill`, `get`, `length` (get-свойство), `join`, `pop`, `push`, `reverse`, `set`, `shift`, `slice`, `sort`, `splice`, `unshift`
...
В этом случае значение счётчика сохраняется, и вместо максимального значения устанавливается тот адрес, 
на котором он в данный момент находится.
