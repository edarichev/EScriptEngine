...
## Вызов функции скрипта из кода на C++
...
Например, метод `Array.filter` принимает в качетсве аргумента фильтрующую функцию.
Вызов функции скрипта из объекта, который работает внутри этого же скрипта,
может быть обработан так:
```C++
// функция передана в качестве аргумента, мы его извлекли в переменную argFunction
...
for (auto &pval : _items) {    // допустим, надо применить эту функцию ко всем элементам 
    ...
    p->setPC((size_t)-1);      // ставим максимально число для остановки выполнения
    ...
    p->machine()->run();       // запускаем с текущего PC, выполненеи остановится
                               // как только PC достигнет предела памяти
    // после завершения run() в стеке мы получим результат
    StackValue v = p->popFromStack();
    bool b = v.getBoolValue(); // предположим, она возвращает логическое значение
    if (b) {
